task :update_location => :environment do
	...
	Inquiry.where(:status => "FRESH").update_all(:status => "Fresh")
	Inquiry.where(:status => "CONTACTED").update_all(:status => "Contacted")
	Inquiry.where(:status => "APPOINTMENT").update_all(:status => "Appointment")
	Inquiry.where(:status => "WORKINGDEAL").update_all(:status => "Working_Deal")
	Inquiry.where(:status => "DEADDEAL").update_all(:status => "Dead_Deal")
	Inquiry.where(:status => "ATTEMPTED").update_all(:status => "Attempted")
	Inquiry.where(:status => "BOUGHTHERE").update_all(:status => "Bought_Here")
	Inquiry.where(:status => "ONHOLD").update_all(:status => "On_Hold")
	Inquiry.where(:status => "BOUGHTELSEWHERE").update_all(:status => "Bought_Else_Where")
	Inquiry.where(:status => "DEALERVISITFOLLOWUP").update_all(:status => "Dealer_Visit_Followup")
	Inquiry.where(:status => "MISSEDAPPOINTMENTFOLLOWUP").update_all(:status => "Missed_Appointment_Followup")
	Inquiry.where(:status => "SPAM").update_all(:status => "Spam")
end
